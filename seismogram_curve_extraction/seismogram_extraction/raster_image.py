...
if __name__ == "__main__":
    ...
    # Hyperparameters
    factor = 1  # Downsample sclicing factor

    # Load, resize, and display each image
    for source_directory in source_directories:
        for image_file in os.listdir(source_directory):
            if image_file.endswith('.jpg') and not image_file.endswith('_binary.jpg'):
                ...
                # Apply global Otsu thresholding
                raster_image.apply_otsu_thresholding(save=True, directory=source_directory, save_name=os.path.basename(image_file).removesuffix('.jpg') + "_binary.jpg")
