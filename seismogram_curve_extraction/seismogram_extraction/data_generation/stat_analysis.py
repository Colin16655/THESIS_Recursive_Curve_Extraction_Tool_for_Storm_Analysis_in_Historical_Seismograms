...
class SeismogramAnalysis:
    ...
    def read_MSEED_batch(self, start_time, end_time, folder, mute=False, plot_batch=False):
        """
    ...
    def process_batches(self, downsampling_fac=10, bandwidth_0=None, bandwidth=None, MSEED_folder=r"seismogram_extraction\data_generation\mseed_files"):
        """
        Process the seismogram in batches, compute the DFT for each batch, 
        and update the PDFs of A_k and B_k.

        bandwidht_0 is for A_0 and B_0
        bandwidht is for A_k and B_k, k = 1, ..., N-1
        ...
        batch_signal = self.read_MSEED_batch(batch_start, batch_end, folder=MSEED_folder)
...
if __name__ == "__main__": 
    ...
    folder_path = sanitize_filename(directory + r"\{}_{}_{}_{}_{}_{}_{}_{}_{}".format(network,
                                                                   station,
                                                                   location,
                                                                   channel,
                                                                   start_time,
    ...
    filepath = folder_path + r"\mseed_files_PDFs" + ".pkl"
    if os.path.exists(filepath):
    ...
    # Plot the PDFs
    folder = folder_path + r"\plot\PDFs"
    # Ensure the directory exists
